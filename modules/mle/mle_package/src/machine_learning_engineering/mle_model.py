...
class MLEModel:
    ...
    def fit(self, data: Any):
        ...
        self.logger.info(
            f"Fitting model with data shape: {getattr(data, 'shape', 'unknown')}"
        )
        raise NotImplementedError()
    ...
    def predict(self, features: Any) -> int:
        ...
        self.logger.info(
            f"Predicting with features shape: {getattr(features, 'shape', 'unknown')}"
        )
        raise NotImplementedError()
    ...
    def log_to_storage(self, client_idx: int, predicted_label: int):
        ...
        """
        self.logger.info(
            f"Logging prediction to storage: client {client_idx}, label {predicted_label}"
        )
        try:
